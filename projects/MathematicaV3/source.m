...
(*                    We demonstrate the resulting triangle structures by starting with an icosahedron, then subdividing its faces into*)
(*                    new triangles and finally carry out four times the orthogonal subdivision using orthogonalSubdi vision.*)
...
(*                    Coloring the resulting triangles according to their longest edge and mapping all polygons to a sphere results in*)
(*                    the following picture.*)
(*                    ln[44] := ( * length of the longest edge of a triangle *)*)
...
(*                    2.3 Some More Complicated Three - Dimensional Graphics 849*)
(*                    We could now refine the last cover picture by adding curvature to the individual pieces of the last picture. Here,*)
(*                    this is implemented.*)
(*                    ln[SO] := HyperbolicDodecahedron [*)
...
(*  \[Bullet] In general, the first face does not lie parallel to the x, y - plane, and therefore we have to generalize the reflection*)
(*code.*)
(*  \[Bullet] The center of the polyhedron is not, in general, {0, 0, 0} . Thus, the process of rotation of the individual parts of*)
(*a face to get the whole face has to be generalized.*)
(*  \[Bullet] The radius of the external sphere that indents the faces has to be defined automatically from the vertices. Here,*)
(*we can introduce a free parameter radRed. It defines the ratio of the radius of the indenting sphere to the length*)
(*of the line segment from the center point of the face to the vertex. In this connection, we now create an exact*)
(*spherical shell.*)
(*  Taking account of these generalizations, removing the display of the intermediate pieces, and putting everything *)
(*2.3 Some More Complicated Three - Dimensional Grophics 851 ----------------------------------*)
(*into a Module, we get the following function : HyperbolicPlatonicSolid [ platonicSolid, depth,*)
(*     graphicsOptions].*)
(*    For the sake of elegance, we make some of the calculations inside the Module more efficient as compared with*)
(*the above implementation (which was easier to present and discuss).*)
...
(*   Here are the remaining hyperbolic Platonic bodies, each indented differently. (Because of the different forms of*)
(*   the bodies, SphericalRegion- >True does not help to make all bodies of the same size; the bounding box*)
(*   not shown will always fit into the displayed volume.)*)
...
(*   ln[56]:= Needs [\[Bullet]Graphics'Polyhedra'\[Bullet]]*)
...
(*   8.53*)
(*   The last HyperbolicPlato code also allows us to view the transition between a " hyperbolic " and a*)
(*   " parabolic " Platonic solid.*)
(*   ln[59]:= (CellPrint[Cell[" o \[Bullet] <> ToString[#] <> "transition:", "Text\[Bullet]]];*)
